...
logging.basicConfig(filename="ArkMailer.log", level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
# Load .env file
load_dotenv()
...
base_url = f"{WISA_URL}/QUERY/OUDERMLR_N?werkdatum={formatted_date}&instellingsnummer={INSTELLINGSNUMMERS}"
...
# De functie zet de Informat Klascode (bv. 'arkls-1blauw')
    # om in het gewenste Google Groepsadres (bv. '1blauw.ouders@ark123.be').
    
    # 1. Gebruik de volledige klascode en verwijder enkel spaties.
    group_prefix = class_code.replace(" ", "")

    # 2. Converteer de prefix naar kleine letters voor standaardisatie.
    group_prefix_lower = group_prefix.lower()

    # 3. Definieer de prefixes die we willen verwijderen.
    prefixes_to_remove = ["arkls-", "arkks-"]
    
    # 4. Loop door de prefixes en verwijder de eerste die matcht.
    cleaned_prefix = group_prefix_lower
    for prefix in prefixes_to_remove:
        if cleaned_prefix.startswith(prefix):
            # De prefix wegsnijden
            cleaned_prefix = cleaned_prefix[len(prefix):]
            # Zodra er één is gevonden en verwijderd, stoppen we de loop.
            break

    # 5. Finale Output Formaat: [SchoonGemaakteKlascode].ouders@[domainname]
    # Retourneert bijvoorbeeld: 1blauw.ouders@ark123.be
...
def group_mailaddresses_by_json(data):
    ...
    for student in data:
        ...
        if student.get("TYPE", "").lower() == "lln    ":
            ...
            # -----------------------------------------------------------------
            # --- [!!! TIJDELIJKE TEST FILTER !!!] ---
           # TEST_KLAS = "ArkLS-1 blauw"  # VUL HIER DE EXACTE KLASCODE IN DIE JE WIL TESTEN
            #if class_code != TEST_KLAS:
...
def get_google_groups(service):
    # Function to retrieve all groups in the domain that follow the required naming convention
    try:
        group_mapping = {}
        page_token = None
        ...
        while True:
            ...
            # Using 'customer=my_customer' retrieves groups within your entire domain
            ...
            for group in groups:
                ...
                # --- CORRECTE FILTER LOGICA VOOR KRAAL.BE ---
                # Gebruikt nu correct de globale variabele DOMAIN_NAME
                if group_email.endswith(f".ouders@{domain_name}"): 
            ...
            page_token = response.get('nextPageToken') # Necessary for large organizations
            if not page_token:
                break  # No more pages
